...
public interface AccessStatisticsRepository extends JpaRepository<AccessStatistics, Long> {
    ...
    List<AccessStatistics> findByAccessDateTimeBetween(LocalDate startDate, LocalDate endDate);

    // 특정 연도와 월의 데이터를 조회 (주별 통계)
    List<AccessStatistics> findByAccessDateTimeBetweenAndAccessDateTimeAfter(LocalDate startDate, LocalDate endDate);
}
